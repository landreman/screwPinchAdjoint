function grid = ChebyshevGrid(N, xmin, xmax)
...
C(:,1) = C(:,1)/2; C(:,N) = C(:,N)/2;
  ...
  Z(L) = zeros(N,1);                      % with zeros on the diagonal.
     ...
     M = 2; % MJL: make 1st and 2nd derivative matrices
for ell = 1:M
...
x_new = (x(end:-1:1) + 1) * 0.5 *delta+xmin;
grid = struct('N',N,'x',x_new,'ddx',-DM(:,:,1)/(0.5*delta),'d2dx2',DM(:,:,2)/(0.25*delta*delta));
